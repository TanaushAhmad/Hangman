...
const Hangman = () => {
  ...
  const drawHangman = () => {
    const canvas = canvasRef.current; if (!canvas) return; const ctx = canvas.getContext('2d'); ctx.clearRect(0, 0, canvas.width, canvas.height); ctx.strokeStyle = '#667eea'; ctx.lineWidth = 3; ctx.lineCap = 'round';
  ...
  const renderWord = () => {
    ...
    return currentShow.title.split('').map((char, index) => {
      ...
      return (
        <div key={index} style={{ width: '45px', height: '60px', border: isRevealed ? (isPreFilled ? '3px solid #7fba00' : '3px solid #667eea') : '3px solid #667eea', borderRadius: '8px', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: '1.8em', fontWeight: 'bold', background: isRevealed ? (isPreFilled ? 'linear-gradient(135deg, #7fba00, #a4de02)' : 'linear-gradient(135deg, #667eea, #764ba2)') : 'white', color: isRevealed ? 'white' : '#667eea', transition: 'all 0.3s' }}>
          {isRevealed ? char : ''}
        </div>
      );
    });
  ...
  const styles = { container: { fontFamily: "'Segoe UI', Tahoma, Geneva, Verdana, sans-serif", background: 'linear-gradient(135deg, #667eea 0%, #764ba2 100%)', minHeight: '100vh', display: 'flex', justifyContent: 'center', alignItems: 'center', padding: '20px' }, main: { background: 'white', borderRadius: '20px', boxShadow: '0 20px 60px rgba(0, 0, 0, 0.3)', maxWidth: '1100px', width: '100%', padding: '30px' } };
        ...
        {currentShow && (
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1.2fr', gap: '25px' }}>
            <div style={{ background: '#f9f9f9', borderRadius: '15px', padding: '20px', display: 'flex', flexDirection: 'column', alignItems: 'center' }}>
              <canvas ref={canvasRef} width="300" height="350" style={{ border: '3px solid #667eea', borderRadius: '10px', background: 'white' }} />
              <div style={{ marginTop: '15px', display: 'flex', gap: '8px' }}>
                {[...Array(maxWrongGuesses)].map((_, i) => (
                  <div key={i} style={{ width: '30px', height: '30px', background: i < wrongGuesses ? '#d13438' : '#7fba00', borderRadius: '50%', display: 'flex', alignItems: 'center', justifyContent: 'center', color: 'white', fontWeight: 'bold' }}>{i < wrongGuesses ? '✗' : '♥'}</div>
                ))}
              </div>
            </div>
            ...
            <div style={{ background: '#f9f9f9', borderRadius: '15px', padding: '20px', display: 'flex', flexDirection: 'column' }}>
              <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '15px' }}>
                <div style={{ background: 'linear-gradient(135deg, #667eea, #764ba2)', color: 'white', padding: '8px 15px', borderRadius: '20px', fontSize: '0.85em', fontWeight: 'bold' }}>{currentShow.category.toUpperCase()}</div>
              ...
              <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(45px, 1fr))', gap: '6px', marginBottom: '20px' }}>
                {letters.map(letter => {
                  ...
                  return (
                    <button key={letter} onClick={() => guessLetter(letter)} disabled={!gameActive || isGuessed} style={{ padding: '12px 8px', border: 'none', borderRadius: '8px', fontSize: '1em', fontWeight: 'bold', cursor: 'pointer', transition: 'all 0.3s', background: isGuessed ? '#e0e0e0' : (isCorrect ? 'linear-gradient(135deg, #7fba00, #a4de02)' : (isWrong ? 'linear-gradient(135deg, #d13438, #ff6b6b)' : 'linear-gradient(135deg, #667eea, #764ba2)')), color: isGuessed ? '#999' : 'white', boxShadow: '0 4px 6px rgba(0, 0, 0, 0.1)' }}>{letter}</button>
                  );
                })}
              </div>

              <div style={{ display: 'flex', gap: '10px', justifyContent: 'center', flexWrap: 'wrap' }}>
                <button onClick={() => { if (gameActive && window.confirm('Start a new game? Current progress will be lost.')) { resetGame(); } else if (!gameActive) { resetGame(); } }} style={{ padding: '12px 25px', border: 'none', borderRadius: '8px', fontSize: '1em', fontWeight: 'bold', cursor: 'pointer', transition: 'all 0.3s', background: '#667eea', color: 'white' }}>New Game</button>
        ...
        {showModal && (
          <div style={{ display: 'flex', position: 'fixed', top: 0, left: 0, width: '100%', height: '100%', background: 'rgba(0, 0, 0, 0.85)', justifyContent: 'center', alignItems: 'center', zIndex: 1000 }}>
            <div style={{ background: 'white', padding: '40px', borderRadius: '20px', textAlign: 'center', maxWidth: '550px', maxHeight: '90vh', overflowY: 'auto' }}>
              <h2 style={{ color: '#667eea', marginBottom: '20px', fontSize: '2em' }}>{modalContent.title}</h2>
              <div>{modalContent.body}</div>
              <button onClick={modalContent.action} style={{ padding: '12px 25px', border: 'none', borderRadius: '8px', fontSize: '1em', fontWeight: 'bold', cursor: 'pointer', transition: 'all 0.3s', background: '#667eea', color: 'white', marginTop: '20px' }}>{modalContent.title.includes('Welcome') ? 'Start Game' : modalContent.title.includes('VICTORY') ? 'Play Again' : modalContent.title.includes('Out of Lives') ? 'New Game' : 'Continue'}</button>
            </div>
          </div>
        )}
      </div>
